from functools import wraps

LIST_OF_ADMINS = [374464076]
LIST_OF_DEKANAT = [374464076, 367567988]
LIST_OF_TEACHERS = [374464076]
#


def restricted_studdekan(func):
    @wraps(func)
    def wrapped(message):
        user_id = message.from_user.id
        if user_id not in LIST_OF_ADMINS:
            print(f'############################# Denied restricted_studdekan for {user_id} #############################')
            return
        return func(message)
    return wrapped


def restricted_dekanat(func):
    @wraps(func)
    def wrapped(message):
        user_id = message.from_user.id
        if user_id not in LIST_OF_DEKANAT:
            print(f'############################# Denied restricted_dekanat for {user_id} #############################')
            return
        return func(message)
    return wrapped


def restricted_teacher(func):
    @wraps(func)
    def wrapped(message):
        user_id = message.from_user.id
        if user_id not in LIST_OF_TEACHERS:
            print(f'############################# Denied restricted_teacher for {user_id} #############################')
            return
        return func(message)
    return wrapped
